...
# MCS 275 Spring 2024
# Adapted from 2022-2023 course materials
...
def random_treeify(keys, R=None):
    ...
    object `R` to provide the entropy (if given).  USUALLY DOES NOT
...
def random_bst_and_node(nodes=6, seed=None):
    """Return a random BST and one of its nodes"""
...
def random_bst_and_node_pair(nodes=6, seed=None, distinct=False):
    """Return a random BST and two of its nodes.  If
    `distinct` is True, the two nodes returned will be distinct."""
    if distinct and nodes < 2:
        raise ValueError(
            "Impossible to select two distinct nodes in a tree with {} nodes".format(
                nodes
            )
        )
